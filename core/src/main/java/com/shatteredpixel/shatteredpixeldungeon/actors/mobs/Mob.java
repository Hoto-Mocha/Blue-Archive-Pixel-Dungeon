...
public abstract class Mob extends Char {
	...
	public void die( Object cause ) {
		...
		if (cause == Dungeon.hero) {
			...
			ThrowingSet throwingSet = Dungeon.hero.belongings.getItem(ThrowingSet.class);
			if (throwingSet != null) {
				throwingSet.reload();
			}

			if (hero.hasTalent(Talent.ENERGY_COLLECT)) {
				ArtifactRecharge.chargeArtifacts(hero, hero.pointsInTalent(Talent.ENERGY_COLLECT)/2f);
				ScrollOfRecharging.charge(hero);
			}

			if (hero.buff(Teleport.TeleportCooldown.class) != null) {
				hero.buff(Teleport.TeleportCooldown.class).kill();
			}

			if (hero.buff(BlastMissile.class) != null) {
				hero.buff(BlastMissile.class).mobsUpdate(this);
			}
		}

		InfiniteBullet infiniteBullet = Dungeon.hero.buff(InfiniteBullet.class);
		if (cause == Dungeon.hero && infiniteBullet != null && Dungeon.hero.hasTalent(Talent.LARGE_MAGAZINE_2)) {
			infiniteBullet.add();
		}
		...
		Laser.LaserCooldown laserCooldown = Dungeon.hero.buff(Laser.LaserCooldown.class);
		if (laserCooldown != null) {
			laserCooldown.kill();
		}

		InfiniteBullet.infiniteBulletCooldown infiniteCooldown = Dungeon.hero.buff(InfiniteBullet.infiniteBulletCooldown.class);
		if (infiniteCooldown != null) {
...
//		}
	...
	public String info(){
		...
		for (Buff b : buffs(QuestEnemy.class)){
			desc += "\n\n_" + Messages.titleCase(b.name()) + "_\n" + b.desc();
		}
	...
	protected class Sleeping implements AiState {
		...
		public boolean act( boolean enemyInFOV, boolean justAlerted ) {
			...
			if (enemyInFOV) {
				...
				if (enemy instanceof Hero && ((Hero) enemy).hasTalent(Talent.QUIET_MOVE)){
					if (Dungeon.level.distance(pos, enemy.pos) >= 4 - ((Hero) enemy).pointsInTalent(Talent.QUIET_MOVE)) {
